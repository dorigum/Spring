...
public class BookService {
	...
	String clientId = "KQqOvALp5pRwi4m0VG90"; //애플리케이션 클라이언트 아이디값"
    String clientSecret = "yFFPquM2oK"; //애플리케이션 클라이언트 시크릿값"
    ...
    public List<BookVO> searchBook(String keyword, int display, int start){
		...
		while(eventType != XmlPullParser.END_DOCUMENT) {
			...
			case XmlPullParser.START_DOCUMENT: // 문서의 시작이면 list 생성
				list = new ArrayList<BookVO>();
				break;
				
			case XmlPullParser.START_TAG:{ // 태그 시작이면
				String tag = parser.getName(); // 파서를 통해 태그 이름을 알아오기
				switch(tag) { // 태그 확인해서
				case "item": // item이면 book 생성
					book = new BookVO(); break;
				case "title": // 제목부터 데이터 가져오기
